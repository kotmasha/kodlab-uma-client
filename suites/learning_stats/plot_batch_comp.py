...
def imp_check(x,y,ind,typ):
    ...
    if typ=='_Q': #qualitative implication (zero threshold)
        ...
        return XY_<min(total*threshold(ind),XY,X_Y_,X_Y) or (XY_==0 and X_Y==0)
...
for typ in ORDERED_TYPES:
    for ind in xrange(NRUNS):
        for t in xrange(DURATION):
            ...
            WEIGHTS[typ][ind].append(convert_weights(DATA[('obs'+typ,'weights')][ind][t]['minus']))
...
ax_imps.set_ylim(bottom=0,top=pow(NSENSORS,2))

#- form the implications plot
t=xrange(DURATION)
for typ in ORDERED_TYPES:
    raw_diffs=np.array(RAW_DIFFS[typ])
    full_diffs=np.array(FULL_DIFFS[typ])
    std_diffs=np.array(STD_DIFFS[typ])
    #means over runs
    ...
    ALPH=1 #foreground transparency coefficients
    BETA=0.2 #background transparency coefficients

    plt.fill_between(t,diff_raw_mean-diff_raw_sdv,diff_raw_mean+diff_raw_sdv,alpha=BETA,color=AGENT_TYPES[typ][1])
    plt.fill_between(t,diff_full_mean-diff_full_sdv,diff_full_mean+diff_full_sdv,alpha=BETA,color=AGENT_TYPES[typ][1])
    plt.fill_between(t,diff_std_mean-diff_std_sdv,diff_std_mean+diff_std_sdv,alpha=BETA,color=AGENT_TYPES[typ][1])

    ax_imps.plot(t,diff_raw_mean,linestyle='solid',linewidth=3,color=AGENT_TYPES[typ][1],alpha=ALPH,label='Learned PCR vs. Ground PCR, '+AGENT_TYPES[typ][0])
    ax_imps.plot(t,diff_full_mean,linestyle='dashed',linewidth=3,color=AGENT_TYPES[typ][1],alpha=ALPH,label='Full implications vs. Ground PCR, '+AGENT_TYPES[typ][0])
    ax_imps.plot(t,diff_std_mean,linestyle='dotted',linewidth=2,color=AGENT_TYPES[typ][1],alpha=ALPH,label='Full implications vs. Set implications, '+AGENT_TYPES[typ][0])
